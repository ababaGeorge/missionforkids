...
export default function ChildNotif() {
  ...
  // 掛載時讀 users/{uid}.notifLastReadAt 當初值（登入 uid）。
  useEffect(() => {
    if (!uid) return;
    firestore()
      .collection('users')
      ...
      .catch((e) => console.warn('[Notif] read lastRead failed', (e as any)?.code));
  }, [uid]);
  ...
  // 未讀 = 通知時間(reviewedAt) > 已讀水位；水位為 null 時全部未讀。
  // 本地單則已讀（localReadIds）額外覆蓋，僅本次 session。
  const isUnread = (n: NotifItem) =>
    !localReadIds.has(n.id) && (lastReadMs == null || toMillis(n.reviewedAt) > lastReadMs);

  const unreadCount = notifs.filter(isUnread).length;

  const markAllRead = () => {
    if (!uid) return;
    // 樂觀水位取「當前可見通知的最大時間戳」，與 isUnread 比較的 reviewedAt 同為 server 時鐘，
    // 避免裝置時鐘偏移導致漏標新通知或剛按的已讀仍顯示未讀。列表為空時退回 Date.now()（無通知可比）。
    const maxMs = notifs.reduce((max, n) => Math.max(max, toMillis(n.reviewedAt)), 0);
    ...
    firestore()
      ...
      .catch((e) => console.warn('[Notif] markAllRead failed', (e as any)?.code));
  };
