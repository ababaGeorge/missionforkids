...
function RedeemConfirmSheet({
  ...
  // 可能因期間又賺點而失真，但總比沒有好）。
  const balanceBeforeSnapshot = order.order.balanceBeforeSnapshot;
  const balanceAfterSnapshot = order.order.balanceAfterSnapshot;
  const hasSnapshot = balanceBeforeSnapshot != null && balanceAfterSnapshot != null;
  ...
  const beforeBalance = hasSnapshot
    ...
    : balance != null
      ? balance + cost
      : null;
  const afterBalance = hasSnapshot ? balanceAfterSnapshot : balance;
