...
合約：小孩 client 先 `createUserWithEmailAndPassword` 拿 uid，再呼叫 `acceptFamilyInvite({ inviteId })`。transaction 內：驗證 invite pending 且未過期；**驗證登入者 email == invite.email（授權檢查，擋 inviteId 外洩冒用，過期檢查之後做）**；產生 `childId`（= 當下 uid）；建 `users/{uid}`(child, childId)、`familyMemberships/{uid}_{familyId}`(childId, active, 帶 invite.childProfile 的 nickname/avatar)、`pointWallets/{familyId}_{childId}`(balance 0)；標記 invite `accepted` + `acceptedBy`。回傳 `{ familyId, childId }`。冪等：同一 uid 重呼叫回傳既有。

> **已實作並定案（2026-05-27 code review 決議）：** 加 `INVITE_EMAIL_MISMATCH`（`permission-denied`）檢查 —— 登入者 token email 必須等於 invite.email（小寫比對）。放在過期檢查之後（「未登入/已過期」測試不帶 token email，放前面會誤觸 mismatch）。**連帶要求 Task 7.1 接受畫面的 email 欄位設唯讀**（從 invite 預填、不可改），避免小孩改 email 後 auth 帳號建好卻被 acceptFamilyInvite 擋成孤兒帳號。
