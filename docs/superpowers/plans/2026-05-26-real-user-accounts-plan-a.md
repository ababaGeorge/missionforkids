...
Run（版本經 compat-check 確認，見 `.specs/compatibility-report.md`）。分兩步：先讓 Expo 配 SDK-54 對應的 jest-expo/jest，再裝其餘並鎖版本：
```bash
npx expo install jest-expo jest
npm install -D @testing-library/react-native@^13.3.3 react-test-renderer@19.1.0 react-server-dom-webpack@~19.1.5 @types/jest@^29
```
重點（否則 EAS `npm ci` 會掛）：
- **不要**裝 `@testing-library/jest-native`（已棄用；RNTL v13 內建 matchers）。
- RNTL 要 **v13**（才支援 React 19）；它仍把 `react-test-renderer` 列 peer，故 `react-test-renderer@19.1.0` 要裝（會有 React 19 棄用警告，無害）。
- jest-expo 54.0.14+ 有 `react-server-dom-webpack` peer，補裝 `~19.1.5` 對齊 react 19.1，避免 EAS Build 的 `npm ci` 因 peer 不匹配失敗。

安裝後驗證 peer 已滿足：`npm ls react-server-dom-webpack`（應顯示 19.1.x，無 missing peer）。
