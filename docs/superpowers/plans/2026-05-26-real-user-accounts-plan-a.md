...
Run:
```bash
cd functions && npm install -D jest ts-jest @types/jest firebase-functions-test
```
Expected: 安裝成功，`functions/package.json` devDependencies 出現上述 4 個套件。
...
Run:
```bash
npm install -D jest jest-expo @testing-library/react-native @testing-library/jest-native @types/jest react-test-renderer@19.1.0
```
注意：`react-test-renderer` 版本要對齊 react 19.1.0。
